...
@endpush
	...
	<ul id="parent-{{ $block->block_uid }}" class="accordion items-wrapper sortable container-fluid px-0 striped-list">
	    ...
	    <li class="row item mb-1" data-uid="{{ '#'.$block->present()->anchor('content',$i) }}">
		    ...
		    <div id="{{ $block->block_uid }}-{{$i}}" class="collapse container-fluid" aria-labelledby="{{ $block->block_uid }}-{{$i}}-trigger" data-parent="#parent-{{ $block->block_uid }}" data-uid="{{ $block->block_uid }}-{{$i}}">
										...
										<div class="input-group">
											...
											<span class="input-group-btn">
												...
												<div class="btn-group" role="group" aria-label="images add/remove">
													<div type="div" class="btn btn-success" data-lfm="image" data-input="thumbnail-{{$block->block_uid}}-{{$i}}-small" data-preview="preview-{{$block->block_uid}}-{{$i}}-small" ><i class="fas fa-upload"></i> Profile</div>
					...
					<div class="row">
					    ...
					    <div class="col-6">
					    	<label>{{ $block->present()->contentLabel('role','label') }}</label>
							<input class="form-control" type="text" name="content[{{ $block->block_uid }}][{{$i}}][role]" value="{{ $block->present()->content('role',$i) }}" />
					    </div>

					    <div class="col-6">
					    	<label>{{ $block->present()->contentLabel('email','label') }}</label>
							<input class="form-control" type="text" name="content[{{ $block->block_uid }}][{{$i}}][email]" value="{{ $block->present()->content('email',$i) }}" />
                        </div>
