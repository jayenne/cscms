	...
	@for( $i = 0; $i < $block->present()->itemsMax(); $i++ )
	    ...
	    <li class="row item" data-uid="{{ $block->block_uid }}-{{$i}}">
		    ...
		    <div class="col">
				...
				<label>{{ $block->present()->contentLabel('company') }}</label>
				<input type="text" class="form-control" name="content[{{ $block->block_uid }}][{{$i}}][company]" value="{{ $block->present()->content('company',$i) }}" />
		    	
		    	<label>{{ $block->present()->contentLabel('sector') }}</label>
				<input class="form-control" type="text" name="content[{{ $block->block_uid }}][{{$i}}][sector]" value="{{ $block->present()->content('sector',$i) }}" />

		    	<label>{{ $block->present()->contentLabel('description') }}</label>
				<textarea class="form-control" name="content[{{ $block->block_uid }}][{{$i}}][description]" >{{ $block->present()->content('description',$i) }}</textarea>
